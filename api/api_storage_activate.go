...
func apiStorageActivateHandler(w http.ResponseWriter, r *http.Request) {
	...
	storageDir := filepath.Join(body.Path, body.StorageName)
  ...
  "scripts": {
	...
	if err := os.WriteFile(filepath.Join(storageDir, "package.json"), []byte(packageJSON), 0644); err != nil {
		w.WriteHeader(http.StatusInternalServerError)
		json.NewEncoder(w).Encode(map[string]any{
			"success": false,
			"error":   err.Error(),
	...
	hashDir := filepath.Join(storageDir, body.HashDirectory)
